...
public class InputModeSwitcher {
    ...
    /**
     * User defined key code, used by soft keyboard. 用于 切换第二个符号键盘
     */
    private static final int USERDEF_KEYCODE_SYM_7 = -7;
     ...
     *
     * @return
     * @有效值 MODE_UNSET（未设置输入法模式）、MODE_SKB_CHINESE（中文标准软键盘模式）、MODE_SKB_SYMBOL1_CN
    ...
    public int switchModeForUserKey(int userKey) {
        ...
        } else if (USERDEF_KEYCODE_SYM_7 == userKey) {
            // 系统键：显示“#+=”的键
            if (MODE_SKB_SYMBOL1_EN == mInputMode) {
                newInputMode = MODE_SKB_SYMBOL2_CN;
            } else if (MODE_SKB_SYMBOL2_EN == mInputMode) {
                newInputMode = MODE_SKB_SYMBOL1_EN;
            } else if (MODE_SKB_SYMBOL2_CN == mInputMode) {
    ...
    public boolean tryHandleLongPressSwitch(int keyCode) {
        if (USERDEF_KEYCODE_LANG_2 == keyCode
            ...
            //mImeService.showOptionsMenu();//caojinliang removed settings
            return true;
        }
        return false;
    }
