...
public class SoftKeyboardView extends View {
     ...
     * @param movePress      如果为true，意味着是因为手指触摸移动到该按键。如果为false，意味着是因为手指按下给按键。
     * @return
     */
    public SoftKey onKeyPress(int x, int y,
        ...
        if (mSoftKeyDown.needBalloon()) {
            ...
            mHintLocationToSkbContainer[1] += mOffsetToSkbContainer[1];
            mHintLocationToSkbContainer[1] -= (int) mContext.getResources().getDimension(R.dimen.baloon_text_gap);
    ...
    private void drawSoftKey(Canvas canvas, SoftKey softKey, int keyXMargin,
        ...
        if (null != keyIcon) {
            ...
            int fontHeight = mFmi.bottom - mFmi.top;//字体高度
            float marginY = (softKey.height() - fontHeight) / 2.0f;//（键盘高度 - 字体高度 ）/2
            float y = softKey.mTop + marginY + fontHeight*3/2;
