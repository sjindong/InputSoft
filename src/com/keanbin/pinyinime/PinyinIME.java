...
public class PinyinIME extends InputMethodService {
	...
	private boolean processFunctionKeys(int keyCode, boolean realAction) {
		...
		if (null != mCandidatesContainer && mCandidatesContainer.isShown()
			...
			// 在预报状态下的删除键处理
			android.util.Log.v("caojinliang","processFunctionKeys::mImeState="+mImeState);
			...
			if (keyCode == KeyEvent.KEYCODE_DEL) {
				android.util.Log.v("caojinliang","processFunctionKeys::realAction="+realAction);
				android.util.Log.v("caojinliang","processFunctionKeys::SIMULATE_KEY_DELETE="+SIMULATE_KEY_DELETE);
				if (!realAction)
					return true;
	...
	public boolean onEvaluateFullscreenMode() {
		...
		if(null!=mEditorInfo){
			int v = mEditorInfo.inputType & EditorInfo.TYPE_MASK_VARIATION;
			android.util.Log.v("caojinliang","onEvaluateFullscreenMode::mEditorInfo.inputType & EditorInfo.TYPE_MASK_VARIATION="+v);
			if (v == EditorInfo.TYPE_TEXT_VARIATION_URI
				||v == EditorInfo.TYPE_TEXT_VARIATION_WEB_EDIT_TEXT)
				isFullScreen = false;
		}
    ...
    Button mExtractAction2;
    ExtractedText mExtractedText;
	
	@Override
    public View onCreateExtractTextView() {
		View ExTextView = super.onCreateExtractTextView();
		LayoutInflater inflater = getLayoutInflater();

		mExtractEditText = (ExtractEditText)ExTextView.findViewById(
				com.android.internal.R.id.inputExtractEditText);
		mExtractAction = (Button)ExTextView.findViewById(
				com.android.internal.R.id.inputExtractAction);
		mExtractAction2 = (Button)ExTextView.findViewById(
				com.android.internal.R.id.inputExtractEditButton);
		if (mExtractAction != null) {
			mExtractAccessories = (ViewGroup)ExTextView.findViewById(
					com.android.internal.R.id.inputExtractAccessories);
		}

		//button layout modified
		FrameLayout.LayoutParams buttonParams = (FrameLayout.LayoutParams) mExtractAction.getLayoutParams();
        buttonParams.gravity = Gravity.BOTTOM;
        mExtractAction.setLayoutParams(buttonParams);	

		FrameLayout.LayoutParams button2Params = (FrameLayout.LayoutParams) mExtractAction2.getLayoutParams();
        button2Params.gravity = Gravity.BOTTOM;
        mExtractAction2.setLayoutParams(button2Params);	

		//text layout modified
		LinearLayout.LayoutParams textParams = (LinearLayout.LayoutParams) mExtractEditText.getLayoutParams();
        textParams.gravity = Gravity.BOTTOM;
		textParams.height = (int)getResources().getDimension(R.dimen.extract_text_heigt);
		textParams.leftMargin = (int)getResources().getDimension(R.dimen.extract_text_left_gap);
        mExtractEditText.setLayoutParams(textParams);	
		//mExtractEditText.setTextAlignment(3);
